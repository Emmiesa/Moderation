...
public class CommandFramework implements CommandExecutor {
    ...
    public CommandFramework(Moderation plugin) {
        this.plugin = plugin;
        ...
        for (int i = args.length; i >= 0; i--) {
            ...
            if (commandMap.containsKey(cmdLabel)) {
                ...
                if (!command.permission().equals("") && (!sender.hasPermission(command.permission()))) {
                    sender.sendMessage(CC.translate(Moderation.getInstance().getConfig().getString("no-permission"))); // TODO change
                    return true;
                }
    ...
    private void defaultCommand(CommandArgs args) {
        ...
        if (args.getSender().hasPermission(Moderation.getInstance().getConfig().getString("command.syntax-bypass-perm"))) {
            if (parts.length > 1) {
            ...
            args.getSender().sendMessage(CC.translate(Moderation.getInstance().getConfig().getString("command.anti-syntax-message").replace("{argument}", args.getLabel())));
        }
    }
}
